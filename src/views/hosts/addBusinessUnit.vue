<template>
  <div>
    ...
    <div class="container">
      <div class="form-box">
        <el-form ref="businessunitform" :model="BusinessUnitinfo"  :rules="rules" label-width="80px">
          <el-form-item label="父业务线" prop="parent_unit">
            <el-select v-model="BusinessUnitinfo.parent_unit" placeholder="请选择业务线">
              <el-option v-for="item in BusinessUnitData" :key="item.name" :value="item.name"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="业务线名" prop="name">
            <el-input v-model="BusinessUnitinfo.name"></el-input>
          </el-form-item>
          <el-form-item label="业务线描述" prop="desc">
            <el-input type="textarea" rows="5" v-model="BusinessUnitinfo.desc"></el-input>
          </el-form-item>
          <el-form-item>
...
<script>
  ...
  import {addBusinessUnit}  from '../../api/api'
export default {
  name: 'addBusinessUnit',
  data() {
    return {
      BusinessUnitData: this.$store.state.businessunitsdata,
      BusinessUnitinfo: {
        parent_unit: '',
        name: '',
        desc: '',
  ...
  methods:{
    onSubmit(Formname) {
      ...
      this.$refs[Formname].validate((valid) => {
        if (valid) {
          addBusinessUnit(this.BusinessUnitinfo)
            .then(res=>{
